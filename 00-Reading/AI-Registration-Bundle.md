...
**Canonical sources:** This bundle is a snapshot of the following files from the Bounded Power Framework repository (https://github.com/BoundedPowerFoundation/Bounded-Power-Framework). The repository copies are authoritative. If this bundle and the repository disagree, the repository wins.

- Registration Form v4 — canonical path: `02-Forms/01-Registration-v4.md`
...
- Registration Process Methodology — companion document

---

# How to use this bundle

This is a single-file package that lets you produce a completed Bounded Power Framework registration for any authority, using any general-purpose AI tool (Claude, ChatGPT, Gemini, etc.) with no prior framework knowledge.

**To use it:**

1. Open an AI tool.
2. Paste the entire contents of this bundle into a new conversation.
3. Replace `[AUTHORITY NAME]` in Part 1 below with the institution you want to register (e.g., "Cuyahoga County Corrections Center" or "Springfield Hospital").
4. Send.

The AI will produce a completed registration form using publicly available information, save it as a downloadable markdown file, and label it `TEST / NOT AN OFFICIAL REGISTRATION` per current framework requirements.
...
- **Part 3** — The Registration Form (v4). The actual form to be filled out.
...
Parts 2–5 are reference material for the AI. You do not need to read them.

---

# PART 1 — Instructions for the AI

You are being asked to produce a completed Bounded Power Framework registration for **[AUTHORITY NAME]** using publicly available information.

This document is a worked example, not a real registration. The output must be labeled `TEST / NOT AN OFFICIAL REGISTRATION` throughout.
...
- Part 3: the registration form itself (v4, 13 sections)
...
2. **Do not pause for human review.** Produce the full registration in one pass. Where the methodology describes a Step 2 human checkpoint, replace it with in-form annotations: when you would have asked the human a clarifying question, instead note your choice and the alternative directly in the relevant field of the form using the format `ANNOTATION: [your note]`. A human reviewer will read the completed form and address annotations afterward.
...
5. **Use the dull-truth register** described in Part 2 (Step 4):
   ...
   - No opinion-forcing adjectives: avoid "extracts," "predatory," "regressive," "captured," "hostile," and similar — they pre-load a conclusion the framework's structure should surface on its own
...
8. **Output format.** Produce the completed registration as a single markdown file the user can download. Use:
   - YAML frontmatter with `mechanism_id: SCBP-REG-TEST-[SHORT-ID]`, `status: TEST_EXAMPLE`, `label: TEST / NOT AN OFFICIAL REGISTRATION`
   - The full v4 form structure, with every field filled per the rules above
...
*The step-by-step method for filling out the registration form. The AI should read this in full before drafting.*
...
# PART 3 — The Registration Form (v4)

*The form itself. The AI reproduces this structure exactly when producing the completed registration.*
...
This form is the public declaration through which an authority enters the Bounded Power Framework. Completing it produces a Mechanism Record, which becomes a permanent public document. The record cannot be edited substantively after submission, only updated through the registry's amendment process.

The form is organized around what the framework wants visible — not around how authorities typically describe themselves. Some sections will ask you to think about your authority in ways your own internal documents may not. That is intentional. The framework's structural logic surfaces patterns of capture, drift, and proxy substitution that are easy for an authority to miss when looking from inside.

Each section begins with a short *diagnostic prompt* — questions to consider before answering. The prompts are not part of the record; they are scaffolding for thinking. The record is what you write in the answer fields.

Where a section references specific constitutional rules (e.g., "R12, R39"), those references are footnoted at the end of the section. They are provided so you can read the underlying rule if you want context. They are not a citation requirement.

To submit: open a GitHub Issue using the template, or email `BoundedPowerFoundation@proton.me` with subject line `REGISTRATION REQUEST`.

---

## Before you begin — three things to know

1. **Your declarations are public the moment they are received.** There is no review queue; submissions are not edited for content. Once filed, a record can be updated but never deleted.

2. **The registry validates structural bounds, not your narrative.** Numerical declarations (response windows, aggregate thresholds, harm threshold values) must fall within the bounds defined in *SCBP-09 — Registry Acceptance Standards*. Out-of-bounds declarations are returned for correction. The registry does not assess whether your descriptive claims are accurate.

3. **What you do not disclose is itself a finding.** This form ends with an explicit honesty assertion. The framework's response to undisclosed-but-discoverable information is structural, not punitive — but the gap becomes a public finding when it surfaces.
...
*End of bundle. The AI should now proceed with the registration per Part 1 instructions.*
